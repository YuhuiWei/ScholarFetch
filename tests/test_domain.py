...
def test_keyword_classify_biology():
    assert _keyword_classify("single cell RNA sequencing") == "biology"
    assert _keyword_classify("protein folding dynamics") == "biology"
    assert _keyword_classify("CRISPR gene editing") == "biology"


def test_keyword_classify_chemistry():
    assert _keyword_classify("organic synthesis reaction") == "chemistry"
    assert _keyword_classify("polymer catalyst") == "chemistry"
...
async def test_classify_uses_override():
...
async def test_classify_invalid_override_raises():
    with pytest.raises(ValueError, match="Invalid domain category"):
...
async def test_classify_openai_failure_falls_back_to_keywords(monkeypatch):
    ...
    assert result == "biology"  # keyword fallback
